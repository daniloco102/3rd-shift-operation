        ...
        if (typeof firebase !== 'undefined' && firebaseConfig.apiKey) {
            try {
                ...
                db.ref('.info/connected').on('value', (snap) => {
                    if (snap.val() === true) {
                        ...
                        if (syncStatusEl) {
                            ...
                            syncStatusEl.innerHTML = '<i class="fa-solid fa-cloud"></i> Local Storage (Mất kết nối)';
                        }
                    }
                });
            } catch(e) {
                ...
                if (syncStatusEl) {
                    syncStatusEl.className = 'badge badge-warning';
                    syncStatusEl.innerHTML = '<i class="fa-solid fa-cloud"></i> Local Storage';
                }
            }
        }
        ...
        const SUPPLIERS = ["HOA ANH ĐÀO", "VIECCO", "SKT"];
        ...
        const FIXED_REQS = {
            "Ca 20:00 - 6:00": { "HOA ANH ĐÀO": 64, "VIECCO": 52, "SKT": 39 },
            "Ca 21:30 - 6:30": { "HOA ANH ĐÀO": 64, "VIECCO": 52, "SKT": 39 },
        ...
        function safeGetLocalArray(key) {
            try {
        ...
        function safeSetLocal(key, valObj) {
            try {
        ...
        let records = safeGetLocalArray('freelancer_records_v2');
        let customShifts = safeGetLocalArray('custom_shifts');
        let currentHistoryPage = 1;
        const HISTORY_PER_PAGE = 30;

        if (records.length === 0) {
            const oldRecords = safeGetLocalArray('freelancer_records');
            if (oldRecords.length > 0) {
                records = oldRecords;
                safeSetLocal('freelancer_records_v2', records);
            }
        }
        
        records.forEach(r => {
            if (r && r.data && r.data["KIẾN VÀNG"]) {
        ...
        async function fetchCloudData() {
            try {
                const res = await fetch('https://check-luong-daily-default-rtdb.firebaseio.com/freelancer_records.json');
                if (res.ok) {
                    const data = await res.json();
                    if (data) {
                        const parsed = ensureArray(data);
                        if (parsed.length > 0) {
                            records = parsed;
                            records.forEach(r => {
                                if (!r || !r.data) return;
                                ...
                                Object.values(r.data).forEach(d => {
                                    if (d && d.lv > 0 && d.lvM === undefined && d.lvF === undefined) {
                                        const totalPresent = (d.m || 0) + (d.f || 0);
                                        if (totalPresent > 0) {
                ...
                console.warn("HTTP Fallback fetch error:", err);
            }
        }
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            const recDateInput = document.getElementById('record-date');
            if (recDateInput) recDateInput.valueAsDate = new Date();
            
            // Render local data immediately on page load
            updateDashboard();

            // Fast REST API sync fallback
            fetchCloudData();
        ...
        function saveToCloud() {
            ...
            if (db) {
                db.ref('freelancer_records').set(records).catch(err => {
                    console.error("Lỗi lưu Firebase WebSocket:", err);
                });
            }
            fetch('https://check-luong-daily-default-rtdb.firebaseio.com/freelancer_records.json', {
                method: 'PUT',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify(records)
            }).catch(e => console.warn("Lỗi lưu Firebase REST:", e));
        }
        ...
        document.getElementById('btn-add-shift').addEventListener('click', () => {
            ...
            if(newShift && newShift.trim() !== '') {
                ...
                if(!exists) {
                    ...
                    safeSetLocal('custom_shifts', customShifts);
        ...
        function updateFormForShift() {
            ...
            if (shift === "Ca 20:00 - 6:00" || shift === "Ca 21:30 - 6:30") {
                ...
                const siblingRec = records.find(r => normalizeDate(r.date) === normDate && r.shift === siblingShift);
                if (siblingRec) {
                    const tReq = Object.values(siblingRec.data).reduce((acc, d) => acc + (d.req || 0), 0);
                    if (tReq > 0) siblingIsMain = true;
                }
                
                if (siblingIsMain) {
                    fixedReqs = { "HOA ANH ĐÀO": 0, "VIECCO": 0, "SKT": 0 };
                } else {
                    fixedReqs = isSunday ? { "HOA ANH ĐÀO": 29, "VIECCO": 21, "SKT": 20 } : { "HOA ANH ĐÀO": 64, "VIECCO": 52, "SKT": 39 };
                }
            } else if (shift === "Ca 21:00 - 6:00") {
                fixedReqs = isSunday ? { "HOA ANH ĐÀO": 29, "VIECCO": 21, "SKT": 20 } : { "HOA ANH ĐÀO": 54, "VIECCO": 41, "SKT": 38 };
            }
            ...
            // Existing record check for date and shift
            const existingRec = records.find(r => normalizeDate(r.date) === normDate && r.shift === shift);

            SUPPLIERS.forEach(sup => {
                const reqInput = document.querySelector(`input[data-supplier="${sup}"][data-field="req"]`);
                const resInput = document.querySelector(`input[data-supplier="${sup}"][data-field="res"]`);
                const mInput = document.querySelector(`input[data-supplier="${sup}"][data-field="m"]`);
                const fInput = document.querySelector(`input[data-supplier="${sup}"][data-field="f"]`);

                if (existingRec && existingRec.data && existingRec.data[sup]) {
                    const dData = existingRec.data[sup];
                    if (reqInput) reqInput.value = dData.req || 0;
                    if (resInput) resInput.value = dData.res || 0;
                    if (mInput) mInput.value = dData.m || 0;
                    if (fInput) fInput.value = dData.f || 0;
                } else {
                    if (resInput) resInput.value = '';
                    if (mInput) mInput.value = '';
                    if (fInput) fInput.value = '';
                    if (fixedReqs) {
                        if (reqInput) reqInput.value = fixedReqs[sup] || 0;
                    } else {
            ...
            if (fixedReqs) {
                ...
                if (isGroupedCheckbox) isGroupedCheckbox.checked = false;
                SUPPLIERS.forEach(sup => {
                    const reqInput = document.querySelector(`input[data-supplier="${sup}"][data-field="req"]`);
                    if (reqInput) {
                ...
                if (isGroupedContainer) isGroupedContainer.style.display = 'block';
                const isGrouped = isGroupedCheckbox ? isGroupedCheckbox.checked : false;
                SUPPLIERS.forEach(sup => {
                    const reqInput = document.querySelector(`input[data-supplier="${sup}"][data-field="req"]`);
                    if (reqInput) {
                        if (isGrouped) {
        ...
        // Tự động tính số Nam/Nữ & cập nhật Thống Kê Ca Tự Động
        ...
        function normalizeDate(dStr) {
            ...
            if (dStr.includes('/')) {
                ...
                if (parts.length === 3) {
                    ...
                    let p2 = parts[2];
                    if (p0.length === 4) return `${p0}-${p1}-${parts[2].padStart(2, '0')}`;
                    return `${p2}-${p1}-${p0}`;
                }
                ...
                if (parts.length === 3) {
                    let p0 = parts[0].padStart(2, '0');
                    let p1 = parts[1].padStart(2, '0');
                    let p2 = parts[2].padStart(2, '0');
                    if (parts[0].length === 4) return `${parts[0]}-${p1}-${p2}`;
                    if (parts[2].length === 4) return `${parts[2]}-${p1}-${p0}`;
                }
            }
            return dStr;
        }
        ...
        function getWeekInfo(dStr) {
            ...
            if (dStr instanceof Date) {
                date = dStr;
            } else if (dStr && typeof dStr === 'string' && dStr.includes('-')) {
                const parts = dStr.split('-');
            ...
            const diffDays = Math.round((sunday - firstSunday) / (24 * 60 * 60 * 1000));
            const weekNum = Math.floor(diffDays / 7) + 1;

            const sunDD = String(sunday.getDate()).padStart(2, '0');
            const sunMM = String(sunday.getMonth() + 1).padStart(2, '0');
            const satDD = String(saturday.getDate()).padStart(2, '0');
            const satMM = String(saturday.getMonth() + 1).padStart(2, '0');
            const satYYYY = saturday.getFullYear();

            const sunYYYY = sunday.getFullYear();
            const sundayStr = `${sunYYYY}-${sunMM}-${sunDD}`;

            const label = `Tuần ${weekNum} (${sunDD}/${sunMM} - ${satDD}/${satMM}/${satYYYY})`;
        ...
        function updateDashboard(evt) {
            if (evt && evt.target && evt.target.id === 'top-week-filter') {
                const chartFilterDate = document.getElementById('chart-filter-date');
                if (chartFilterDate) chartFilterDate.value = evt.target.value;
            } else if (evt && evt.target && evt.target.id === 'chart-filter-date') {
                const topWeekFilter = document.getElementById('top-week-filter');
                if (topWeekFilter) topWeekFilter.value = evt.target.value;
            }

            updateFilters();

            const filterDateVal = document.getElementById('chart-filter-date')?.value || document.getElementById('top-week-filter')?.value || 'all';
            const filterTimeframe = document.getElementById('chart-filter-timeframe')?.value || 'week';
            
            globalWeekDates = [];
            globalChartLabels = [];

            if (filterDateVal === 'all') {
                const uniqueDates = [...new Set(records.map(r => r && r.date ? normalizeDate(r.date) : ''))].filter(Boolean).sort((a,b) => a.localeCompare(b));
                if (uniqueDates.length > 0) {
                    uniqueDates.forEach(dStr => {
                        ...
                        globalChartLabels.push(formatDate(dStr));
                    });
                } else {
                ...
                if (filterDateVal && filterDateVal.includes('-')) {
        ...
        function updateFilters() {
            ...
            if (!filterDate || !filterShift) return;
            ...
            if (filterDate.options.length === 0) {
                ...
                if (topWeekFilter) topWeekFilter.addEventListener('change', updateDashboard);
            }
            
            const currTimeframe = filterTimeframe ? filterTimeframe.value : 'week';
            const currDateVal = filterDate.value;
            const currShiftVal = filterShift.value;
            ...
            // Build Top Week Filter
            if (topWeekFilter) {
                const weekMap = {};
                const nowWeek = getWeekInfo(new Date());
                weekMap[nowWeek.sundayStr] = nowWeek.label;

                uniqueDates.forEach(dStr => {
                    const wInfo = getWeekInfo(dStr);
                    if (wInfo) weekMap[wInfo.sundayStr] = wInfo.label;
                });

                const sortedSundayStrs = Object.keys(weekMap).sort((a,b) => b.localeCompare(a));
                let topWeekHtml = '<option value="all">Tất cả các tuần</option>';
                sortedSundayStrs.forEach(sunStr => {
                    topWeekHtml += `<option value="${sunStr}">${weekMap[sunStr]}</option>`;
                });
                topWeekFilter.innerHTML = topWeekHtml;
            }

            // Build Date/Period Options according to Timeframe
            let dateHtml = '';
            if (currTimeframe === 'week') {
                dateHtml = '<option value="all">Tất cả các tuần</option>';
                const weekMap = {};
                const nowWeek = getWeekInfo(new Date());
                weekMap[nowWeek.sundayStr] = nowWeek.label;
                ...
                const sortedSundayStrs = Object.keys(weekMap).sort((a,b) => b.localeCompare(a));
                sortedSundayStrs.forEach(sunStr => {
                    dateHtml += `<option value="${sunStr}">${weekMap[sunStr]}</option>`;
                });
            } else if (currTimeframe === 'month') {
                dateHtml = '<option value="all">Tất cả các tháng</option>';
                const monthMap = {};
                uniqueDates.forEach(dStr => {
                    const parts = dStr.split('-');
                    if (parts.length === 3) {
                        ...
                        const label = `Tháng ${mm}/${yyyy} (01/${mm} - ${lastDay}/${mm}/${yyyy})`;
                        monthMap[key] = label;
                    }
                });
                ...
                const nowKey = `${nowYYYY}-${nowMM}`;
                if (!monthMap[nowKey]) {
                    const lastDay = new Date(nowYYYY, now.getMonth() + 1, 0).getDate();
                    monthMap[nowKey] = `Tháng ${nowMM}/${nowYYYY} (01/${nowMM} - ${lastDay}/${nowMM}/${nowYYYY})`;
                }

                const sortedMonthKeys = Object.keys(monthMap).sort((a,b) => b.localeCompare(a));
                sortedMonthKeys.forEach(mKey => {
                    dateHtml += `<option value="${mKey}">${monthMap[mKey]}</option>`;
                });
            } else {
                dateHtml = '<option value="all">Tất cả các ngày</option>';
                uniqueDates.forEach(dStr => {
                    dateHtml += `<option value="${dStr}">${formatDate(dStr)}</option>`;
                });
            }
            ...
            if (currDateVal && [...filterDate.options].some(o => o.value === currDateVal)) {
                filterDate.value = currDateVal;
            }
            if (currShiftVal && [...filterShift.options].some(o => o.value === currShiftVal)) {
                filterShift.value = currShiftVal;
            }
        }

        function getFilteredRecords() {
            const s = document.getElementById('chart-filter-shift')?.value || 'all';
            const d = document.getElementById('chart-filter-date')?.value || document.getElementById('top-week-filter')?.value || 'all';
            const timeframe = document.getElementById('chart-filter-timeframe')?.value || 'week';
            ...
            return records.filter(r => {
                ...
                if (d !== 'all') {
                    if (timeframe === 'week') {
                        const wInfo = getWeekInfo(normRDate);
                        if (!wInfo || wInfo.sundayStr !== d) return false;
                    } else if (timeframe === 'month') {
                        if (!normRDate.startsWith(d)) return false;
                    } else {
                        if (normRDate !== d) return false;
                    }
                }
        ...
        function updateOverviewCard(resetPeriod = false) {
            const timeframeEl = document.getElementById('overview-filter-timeframe');
            const periodEl = document.getElementById('overview-filter-period');
            const shiftEl = document.getElementById('overview-filter-shift');

            if (!timeframeEl || !periodEl || !shiftEl) return;

            const timeframe = timeframeEl.value;
            const uniqueDates = [...new Set(records.map(r => r.date))].sort((a,b) => b.localeCompare(a));
            const uniqueShifts = [...new Set(records.map(r => r.shift))];

            if (shiftEl.options.length === 0) {
                let shiftHtml = '<option value="all">Tất cả các ca</option>';
                uniqueShifts.forEach(s => shiftHtml += `<option value="${s}">${s}</option>`);
                shiftEl.innerHTML = shiftHtml;
            }

            if (resetPeriod || periodEl.options.length === 0) {
                let periodHtml = '';
                if (timeframe === 'week') {
                    periodHtml = '<option value="all">Tất cả các tuần</option>';
                    const weekMap = {};
                    const nowWeek = getWeekInfo(new Date());
                    weekMap[nowWeek.sundayStr] = nowWeek.label;
                    ...
                    const sortedSundayStrs = Object.keys(weekMap).sort((a,b) => b.localeCompare(a));
                    sortedSundayStrs.forEach(sunStr => {
                        periodHtml += `<option value="${sunStr}">${weekMap[sunStr]}</option>`;
                    });
                } else if (timeframe === 'month') {
                    periodHtml = '<option value="all">Tất cả các tháng</option>';
                    const monthMap = {};
                    uniqueDates.forEach(dStr => {
                        const parts = dStr.split('-');
                        if (parts.length === 3) {
                    ...
                    const sortedMonthKeys = Object.keys(monthMap).sort((a,b) => b.localeCompare(a));
                    sortedMonthKeys.forEach(mKey => {
                        periodHtml += `<option value="${mKey}">${monthMap[mKey]}</option>`;
                    });
                } else {
                    periodHtml = '<option value="all">Tất cả các ngày</option>';
                    uniqueDates.forEach(dStr => {
                        periodHtml += `<option value="${dStr}">${formatDate(dStr)}</option>`;
                    });
                }
            ...
            records.forEach(r => {
                ...
                if (periodVal !== 'all') {
                    if (timeframe === 'week') {
                        const wInfo = getWeekInfo(normRDate);
                        if (!wInfo || wInfo.sundayStr !== periodVal) return;
                    } else if (timeframe === 'month') {
                        if (!normRDate.startsWith(periodVal)) return;
                    } else {
                        if (normRDate !== periodVal) return;
                    }
                }

                if (shiftVal !== 'all' && r.shift !== shiftVal) return;
            ...
            const activeRes = tRes - tLv;
            const rate = tReq === 0 ? 0 : Math.round((activeRes / tReq) * 100);
            const totalGender = tM + tF || 1;
            const mPct = Math.round((tM / totalGender) * 100);
            const fPct = 100 - mPct;

            const bReq = document.getElementById('banner-req-val');
            const bRes = document.getElementById('banner-res-val');
            const bRate = document.getElementById('banner-rate-val');
            const bMale = document.getElementById('banner-male-val');
            const bFem = document.getElementById('banner-female-val');

            if (bReq) bReq.innerText = tReq.toLocaleString('vi-VN');
            if (bRes) bRes.innerText = activeRes.toLocaleString('vi-VN');
            if (bRate) {
                bRate.innerText = `${rate}%`;
                bRate.className = `rate-badge ${rate >= 80 ? 'emerald' : (rate < 60 ? 'danger' : '')}`;
            }
            if (bMale) bMale.innerText = `${tM} (${mPct}%)`;
            if (bFem) bFem.innerText = `${tF} (${fPct}%)`;
        }
        function updateStats() {
            ...
            document.querySelectorAll('.stat-total-req').forEach(el => el.innerText = tReq.toLocaleString('vi-VN'));
            document.querySelectorAll('.stat-total-res').forEach(el => el.innerText = activeRes.toLocaleString('vi-VN'));
            document.querySelectorAll('.stat-avg-rate').forEach(el => el.innerText = `${rate}%`);
            document.querySelectorAll('.stat-total-lv').forEach(el => el.innerText = tLv.toLocaleString('vi-VN'));

            const elGender = document.getElementById('total-gender-ratio');
            if (elGender) elGender.innerText = `${mPct}% (${fPct}%)`;
        ...
        let supplierMiniChartInstances = {};
        let shiftChartInstances = {};

        const SUPPLIER_DISPLAY_NAMES = {
            "HOA ANH ĐÀO": "GHN Express - Hoa Anh Đào",
            "VIECCO": "GHN Express - Viecco",
            "SKT": "SKT"
        };

        function toggleShiftDetailsSection() {
            const body = document.getElementById('shift-details-body');
            const chevron = document.getElementById('shift-detail-chevron');
        ...
        function renderSupplierDetailCards(filtered) {
            const grid = document.getElementById('supplier-cards-grid');
            if (!grid) return;

            Object.values(supplierMiniChartInstances).forEach(c => { if(c) c.destroy(); });
            supplierMiniChartInstances = {};
            ...
            const stats = {};
            SUPPLIERS.forEach(s => {
                stats[s] = { req: 0, res: 0, m: 0, f: 0, lv: 0 };
            });

            filtered.forEach(r => {
                SUPPLIERS.forEach(s => {
                    if (r.data[s]) {
                        stats[s].req += (r.data[s].req || 0);
                        stats[s].res += (r.data[s].res || 0);
                        stats[s].m += (r.data[s].m || 0);
                        stats[s].f += (r.data[s].f || 0);
                        stats[s].lv += ((r.data[s].lvM || 0) + (r.data[s].lvF || 0));
                    }
                });
            });

            let dateTag = '★ Hóa Đơn Chu Kỳ';
            if (globalWeekDates && globalWeekDates.length > 0) {
                const first = globalWeekDates[0].dateStr.slice(0, 5);
                const last = globalWeekDates[globalWeekDates.length - 1].dateStr.slice(0, 5);
                dateTag = `★ ${first} - ${last}`;
            }
            ...
            SUPPLIERS.forEach(s => {
                const st = stats[s];
                const activeRes = st.res - st.lv;
                const rate = st.req === 0 ? 0 : Math.round((activeRes / st.req) * 100);
                const totalGender = st.m + st.f || 1;
                const mPct = Math.round((st.m / totalGender) * 100);
                const fPct = 100 - mPct;
                const dispName = SUPPLIER_DISPLAY_NAMES[s] || s;
                const color = SUPPLIER_COLORS[s] || '#3b82f6';
                const sId = s.replace(/\s+/g, '_');

                cardsHtml += `
                    <div class="supplier-card">
                        <div class="supplier-card-header">
                            <div>
                                <div class="supplier-card-title" style="color: ${color};">${escapeHTML(dispName)}</div>
                                <div class="supplier-card-subtitle">Hiệu suất trong chu kỳ</div>
                            </div>
                            <div class="rate-badge ${rate >= 80 ? 'emerald' : (rate < 60 ? 'danger' : '')}">${rate}% Đạt</div>
                        </div>

                        <div class="supplier-stats-row">
                            <div class="supplier-stat-box">
                                <div class="label">Tổng Yêu Cầu</div>
                                <div class="val">${st.req}</div>
                            </div>
                            <div class="supplier-stat-box">
                                <div class="label">Thực Tế Đến</div>
                                <div class="val" style="color:var(--success);">${activeRes}</div>
                            </div>
                        </div>

                        <div class="gender-breakdown-row">
                            <span>Nam: <b>${st.m} (${mPct}%)</b></span>
                            <span>Nữ: <b>${st.f} (${fPct}%)</b></span>
                        </div>

                        <div class="supplier-mini-chart-container">
                            <canvas id="supplier-mini-chart-${sId}"></canvas>
                        </div>

                        <div class="date-tag-bottom">${dateTag}</div>
                    </div>
                `;
            });
            ...
            SUPPLIERS.forEach(s => {
                const st = stats[s];
                const sId = s.replace(/\s+/g, '_');
                const canvasEl = document.getElementById(`supplier-mini-chart-${sId}`);
                if (!canvasEl) return;

                const ctx = canvasEl.getContext('2d');
                supplierMiniChartInstances[s] = new Chart(ctx, {
                    type: 'bar',
                    data: {
                        labels: ['Chỉ Số'],
                        datasets: [
                    ...
                    options: {
                        ...
                        plugins: {
                            legend: {
                                position: 'bottom',
                                labels: { color: textColor, font: { size: 10 }, usePointStyle: true, boxWidth: 8 }
                            },
                            datalabels: {
                                color: '#ffffff',
        ...
        function renderShiftDetailCharts(filtered) {
            const grid = document.getElementById('shift-details-body');
            if (!grid) return;

            Object.values(shiftChartInstances).forEach(c => { if(c) c.destroy(); });
            shiftChartInstances = {};
            ...
            const shiftMap = {};
            filtered.forEach(r => {
                const shiftName = r.shift || 'Ca không xác định';
                if (!shiftMap[shiftName]) shiftMap[shiftName] = {};
                SUPPLIERS.forEach(s => {
                    if (!shiftMap[shiftName][s]) shiftMap[shiftName][s] = { req: 0, res: 0, lv: 0 };
                    if (r.data[s]) {
                        shiftMap[shiftName][s].req += (r.data[s].req || 0);
                        shiftMap[shiftName][s].res += (r.data[s].res || 0);
                        shiftMap[shiftName][s].lv += ((r.data[s].lvM || 0) + (r.data[s].lvF || 0));
                    }
                });
            });

            const shiftNames = Object.keys(shiftMap);
            if (shiftNames.length === 0) {
                grid.innerHTML = '<div style="color:var(--text-muted); text-align:center; grid-column:1/-1; padding:2rem;">Chưa có dữ liệu theo ca</div>';
                return;
            }

            let dateTag = '★ Hóa Đơn Chu Kỳ';
            if (globalWeekDates && globalWeekDates.length > 0) {
                dateTag = `★ ${globalWeekDates[0].dateStr.slice(0, 5)}`;
            }

            let html = '';
            shiftNames.forEach((shiftName, idx) => {
                const shiftId = 'shift_' + idx;
                html += `
                    <div class="shift-chart-card">
                        <div class="shift-chart-header">
                            <div>
                                <div class="shift-chart-title">| Ca: ${escapeHTML(shiftName)}</div>
                                <div class="shift-chart-subtitle">Chi tiết tỉ lệ đáp ứng (%)</div>
                            </div>
                        </div>
                        <div style="height: 190px; position: relative;">
                            <canvas id="shift-chart-canvas-${shiftId}"></canvas>
                        </div>
                        <div class="date-tag-bottom">${dateTag}</div>
                    </div>
                `;
            ...
            shiftNames.forEach((shiftName, idx) => {
                const shiftId = 'shift_' + idx;
                const canvasEl = document.getElementById(`shift-chart-canvas-${shiftId}`);
                if (!canvasEl) return;

                const sData = shiftMap[shiftName];
                const labels = SUPPLIERS.map(s => s.replace("HOA ANH ĐÀO", "HAD"));
                const rates = SUPPLIERS.map(s => {
                    const st = sData[s];
                    const activeRes = st.res - st.lv;
                    if (st.req === 0) return activeRes > 0 ? 100 : 0;
                    return Math.round((activeRes / st.req) * 100);
                });

                const bgColors = SUPPLIERS.map(s => SUPPLIER_COLORS[s] || '#3b82f6');

                const ctx = canvasEl.getContext('2d');
                shiftChartInstances[shiftId] = new Chart(ctx, {
                    type: 'bar',
                    data: {
                        labels: labels,
                        datasets: [{
                    ...
                    options: {
                        ...
                        scales: {
                            y: { max: 100, beginAtZero: true, grid: { color: gridColor }, ticks: { color: textColor, callback: v => v + '%' } },
        ...
        function updateCharts() {
            const filtered = getFilteredRecords();
            const viewMode = document.getElementById('freelancer-view-mode')?.value || 'detail';
            
            const detailSec = document.getElementById('supplier-detail-section');
            const summarySec = document.getElementById('summary-charts-container');
            
            if (viewMode === 'detail') {
                if (detailSec) detailSec.style.display = 'block';
                if (summarySec) summarySec.style.display = 'none';
                renderSupplierDetailCards(filtered);
                renderShiftDetailCharts(filtered);
                return;
            } else {
                if (detailSec) detailSec.style.display = 'none';
                if (summarySec) summarySec.style.display = 'grid';
            }
            ...
            filtered.forEach(r => {
                const normRDate = normalizeDate(r.date);
                if (supplierStats[normRDate] !== undefined) {
                    SUPPLIERS.forEach(s => {
                        if(r.data[s]) {
                            supplierStats[normRDate][s].req += (r.data[s].req || 0); 
                            supplierStats[normRDate][s].res += (r.data[s].res || 0);
                            supplierStats[normRDate][s].m += (r.data[s].m || 0);
                            supplierStats[normRDate][s].f += (r.data[s].f || 0);
                            const lv = (r.data[s].lvM || 0) + (r.data[s].lvF || 0);
                            supplierStats[normRDate][s].lv += lv;
                        }
                    });
                }
            });
        ...
        function renderGenderRatioTables() {
            ...
            records.forEach(r => {
                ...
                // Table 1: Supplier
                if (isMatchShift && supData[SUPPLIERS[0]] && supData[SUPPLIERS[0]][rDate]) {
                    SUPPLIERS.forEach(s => {
                        if (r.data[s] && supData[s][rDate]) {
                ...
                // Table 2: Shift
                if (shfData[r.shift] && shfData[r.shift][rDate]) {
                    let rM=0, rF=0;
        ...
        function formatDate(dateStr) {
            ...
            const dateObj = new Date(dateStr);
            const suffix = dateObj.getDay() === 0 ? ' (CN)' : '';
        ...
        function generateOpsDates() {
            ...
            for (let i = 0; i < 7; i++) {
                ...
                if (isSunday) {
                    dateString = `<span style="color:var(--danger); font-weight:bold;">${dateString} (CN)</span>`;
                }
        ...
        function renderTargetComparisonTable(week) {
            ...
            const formatW = (w) => w ? w.toString().replace(/Tuần\s*/i, '').trim() : '';
            const pWStr = prevWeek ? formatW(prevWeek) : 'Tuần trước';
            const wStr = formatW(week);
        ...
        function generateTargetAnalysis(currData, prevData, targetFixed) {
            ...
            try {
                const allWeeks = Object.keys(opsData?.targetMetrics || {})
                    .map(w => {
                        const num = parseInt(w.replace(/[^\d]/g, ''));
                        return { key: w, num: num };
                    })
                    .filter(w => !isNaN(w.num))
                    .sort((a,b) => a.num - b.num);
                
                if (allWeeks.length >= 2) {
                    const lastWeeks = allWeeks.slice(-4);
                    const volumes = lastWeeks.map(w => opsData.targetMetrics[w.key]?.sort?.volume || 0);
                    ...
                    if (volumes.length > 1 && volumes[volumes.length-1] > 0) {
                        ...
                        let nextVolume = Math.round(slope * n + intercept);
                        if (nextVolume < 0) nextVolume = 0;
                        const trend = slope > 0 ? "TĂNG" : "GIẢM";
                        let percent = (Math.abs(slope) / volumes[n-1] * 100).toFixed(1);
                        if (percent > 100) percent = ">100";
                        
                        let trendColor = slope > 0 ? "var(--warning)" : "var(--success)";
                        
                        analysis += `<div style="margin-top: 1rem; padding: 0.75rem; background: rgba(99, 102, 241, 0.05); border-left: 4px solid var(--primary); border-radius: var(--radius-md);">
                            <strong style="color: var(--primary);"><i class="fa-solid fa-wand-magic-sparkles"></i> DỰ BÁO AI TUẦN TỚI:</strong><br>
                            Dựa trên đà phát triển của ${n} tuần qua, thuật toán hồi quy Linear Regression dự đoán Volume SORT tuần tới sẽ <span style="color:${trendColor}; font-weight:bold;">${trend} khoảng ${percent}%</span> (Ước tính: ${formatNumber(nextVolume)} đơn).
                            <div style="margin-top: 0.5rem; font-size: 0.85rem; color: var(--text-muted);"><i class="fa-solid fa-lightbulb" style="color:var(--warning)"></i> Hành động: Vui lòng điều chỉnh định biên nhân sự ca 3 để đáp ứng sự biến động này!</div>
                        </div>`;
                    }
                }
            } catch(e) { console.error("AI Forecast Error:", e); }
        ...
        let currentFilteredActive = [];
        let hrResignChartInstance = null;
        let hrNewChartInstance = null;
        let hrOverviewChartInstance = null;

        let onboardChartPrevInstance = null;
        let onboardChartCurrInstance = null;
        let resignedChartPrevInstance = null;
        let resignedChartCurrInstance = null;

        function toggleHRSection(bodyId, chevronId) {
        ...
        function renderHRDashboard() {
            if (!window.HR_DATA) {
                ...
                const tbl = document.getElementById('hr-table-container');
                if (tbl) {
                    tbl.innerHTML = '<div style="padding: 2rem; text-align: center; color: var(--text-muted);">Chưa có dữ liệu nhân sự. Vui lòng chạy file CapNhatNhanSu.bat</div>';
            ...
            const monthSelect = document.getElementById('hr-filter-month');
            ...
            const selShift = shiftSelect ? shiftSelect.value : 'all';
            const defaultMonthKey = `${String(new Date().getMonth()+1).padStart(2,'0')}/${new Date().getFullYear()}`;
            const selMonth = (monthSelect && monthSelect.value) ? monthSelect.value : defaultMonthKey;

            // Parse selected month and previous month
            const monthParts = selMonth.split('/');
            const selM = parseInt(monthParts[0]) || (new Date().getMonth()+1);
            const selY = parseInt(monthParts[1]) || new Date().getFullYear();
            let prevM = selM - 1;
            let prevY = selY;
            if (prevM <= 0) { prevM = 12; prevY -= 1; }
            const prevMonthKey = `${String(prevM).padStart(2,'0')}/${prevY}`;

            // Update Titles
            const titleOnbPrev = document.getElementById('onboard-title-prev');
            const titleOnbCurr = document.getElementById('onboard-title-curr');
            const titleResPrev = document.getElementById('resigned-title-prev');
            const titleResCurr = document.getElementById('resigned-title-curr');
            if (titleOnbPrev) titleOnbPrev.innerText = `Tháng trước (${prevMonthKey})`;
            if (titleOnbCurr) titleOnbCurr.innerText = `Tháng chọn lọc (${selMonth})`;
            if (titleResPrev) titleResPrev.innerText = `Tháng trước (${prevMonthKey})`;
            if (titleResCurr) titleResCurr.innerText = `Tháng chọn lọc (${selMonth})`;

            // Filter Active & Resigned
            const filteredActive = hrActiveData.filter(e => {
                const matchDept = selDept === 'all' || e.dept === selDept;
                const matchShift = selShift === 'all' || e.shift === selShift;
                return matchDept && matchShift;
            });

            const filteredResigned = hrResignedData.filter(e => {
                const matchDept = selDept === 'all' || e.dept === selDept;
                const matchShift = selShift === 'all' || e.shift === selShift;
                return matchDept && matchShift;
            });

            // 1. Build Top 3 Summary Cards
            const activeTotal = filteredActive.length;
            const activeGt50 = filteredActive.filter(e => (e.daysWorked || 0) > 50).length;
            const activeLte50 = activeTotal - activeGt50;
            const activeMale = filteredActive.filter(e => e.gender === 'Nam').length;
            const activeFemale = activeTotal - activeMale;

            const shiftCounts = {};
            filteredActive.forEach(e => {
                const s = e.shift || 'Khác';
                if (!shiftCounts[s]) shiftCounts[s] = [];
                shiftCounts[s].push(e);
            });
            const sortedShifts = Object.keys(shiftCounts).sort((a,b) => shiftCounts[b].length - shiftCounts[a].length);

            let cardsHtml = `
                <div class="stat-card" style="background: var(--bg-card); border: 1px solid var(--border-color); padding: 1.25rem; border-radius: 12px; text-align: center;">
                    <div style="font-size: 0.75rem; font-weight: 700; color: var(--text-muted); text-transform: uppercase; margin-bottom: 0.25rem;">TỔNG NHÂN VIÊN</div>
                    <div style="font-size: 2rem; font-weight: 800; color: var(--text-main); margin-bottom: 0.5rem;">${activeTotal}</div>
                    <div style="display: flex; justify-content: center; gap: 0.5rem; margin-bottom: 0.5rem; flex-wrap: wrap;">
                        <span style="background: rgba(16, 185, 129, 0.15); color: #10b981; padding: 0.2rem 0.6rem; border-radius: 12px; font-size: 0.75rem; font-weight: 700;">>50n: ${activeGt50}</span>
                        <span style="background: rgba(245, 158, 11, 0.15); color: #f59e0b; padding: 0.2rem 0.6rem; border-radius: 12px; font-size: 0.75rem; font-weight: 700;">≤50n: ${activeLte50}</span>
                    </div>
                    <div style="display: flex; justify-content: center; gap: 0.75rem; font-size: 0.8rem; font-weight: 700;">
                        <span style="color: #38bdf8;">Nam: ${activeMale}</span>
                        <span style="color: #ec4899;">Nữ: ${activeFemale}</span>
                    </div>
                </div>
            `;

            for (let i = 0; i < 2; i++) {
                const sName = sortedShifts[i];
                if (sName) {
                    const list = shiftCounts[sName];
                    const sTot = list.length;
                    const sGt50 = list.filter(e => (e.daysWorked || 0) > 50).length;
                    const sLte50 = sTot - sGt50;
                    const sMale = list.filter(e => e.gender === 'Nam').length;
                    const sFem = sTot - sMale;

                    cardsHtml += `
                        <div class="stat-card" style="background: var(--bg-card); border: 1px solid var(--border-color); padding: 1.25rem; border-radius: 12px; text-align: center;">
                            <div style="font-size: 0.75rem; font-weight: 700; color: var(--text-muted); text-transform: uppercase; margin-bottom: 0.25rem;">${sName.toUpperCase()}</div>
                            <div style="font-size: 2rem; font-weight: 800; color: var(--text-main); margin-bottom: 0.5rem;">${sTot}</div>
                            <div style="display: flex; justify-content: center; gap: 0.5rem; margin-bottom: 0.5rem; flex-wrap: wrap;">
                                <span style="background: rgba(16, 185, 129, 0.15); color: #10b981; padding: 0.2rem 0.6rem; border-radius: 12px; font-size: 0.75rem; font-weight: 700;">>50n: ${sGt50}</span>
                                <span style="background: rgba(245, 158, 11, 0.15); color: #f59e0b; padding: 0.2rem 0.6rem; border-radius: 12px; font-size: 0.75rem; font-weight: 700;">≤50n: ${sLte50}</span>
                            </div>
                            <div style="display: flex; justify-content: center; gap: 0.75rem; font-size: 0.8rem; font-weight: 700;">
                                <span style="color: #38bdf8;">Nam: ${sMale}</span>
                                <span style="color: #ec4899;">Nữ: ${sFem}</span>
                            </div>
                        </div>
                    `;
                }
            ...
            const cardsContainer = document.getElementById('hr-active-summary-cards');
            if (cardsContainer) cardsContainer.innerHTML = cardsHtml;

            // 2. Onboard Headcount per Week
            ...
            const onboardPrevData = [0, 0, 0, 0, 0];
            const onboardCurrData = [0, 0, 0, 0, 0];

            filteredActive.forEach(e => {
                if (!e.joinDateStr) return;
                const parts = e.joinDateStr.split('/');
                if (parts.length === 3) {
                    const mStr = parts[1].padStart(2, '0');
                    const mKey = `${mStr}/${parts[2]}`;
                    const w = getWeekOfMonthFromStr(e.joinDateStr);
                    if (w >= 1 && w <= 5) {
                        if (mKey === prevMonthKey) onboardPrevData[w - 1]++;
                        if (mKey === selMonth) onboardCurrData[w - 1]++;
                    }
                }
            });

            // Onboard Chart Prev
            const elOnbPrev = document.getElementById('onboardChartPrev');
            if (elOnbPrev) {
                if (onboardChartPrevInstance) onboardChartPrevInstance.destroy();
                onboardChartPrevInstance = new Chart(elOnbPrev.getContext('2d'), {
                    type: 'bar',
                    data: {
                        labels: labelsPrev,
                        datasets: [{
                            label: 'Onboard',
                            data: onboardPrevData,
            ...
            // Onboard Chart Curr
            const elOnbCurr = document.getElementById('onboardChartCurr');
            if (elOnbCurr) {
                if (onboardChartCurrInstance) onboardChartCurrInstance.destroy();
                onboardChartCurrInstance = new Chart(elOnbCurr.getContext('2d'), {
                    type: 'bar',
                    data: {
                        labels: labelsCurr,
                        datasets: [{
                            label: 'Onboard',
                            data: onboardCurrData,
                            backgroundColor: '#10b981',
                            borderRadius: 6
                        }]
                    },
            ...
            // 3. Resigned Stacked Headcount per Week
            const resignedPrevLte60 = [0, 0, 0, 0, 0];
            const resignedPrevGt60 = [0, 0, 0, 0, 0];
            const resignedCurrLte60 = [0, 0, 0, 0, 0];
            const resignedCurrGt60 = [0, 0, 0, 0, 0];

            filteredResigned.forEach(e => {
                if (!e.leaveDateStr) return;
                const parts = e.leaveDateStr.split('/');
                if (parts.length === 3) {
                    const mStr = parts[1].padStart(2, '0');
                    const mKey = `${mStr}/${parts[2]}`;
                    const w = getWeekOfMonthFromStr(e.leaveDateStr);
                    const isUnder60 = (e.daysWorked || 0) <= 60;

                    if (w >= 1 && w <= 5) {
                        if (mKey === prevMonthKey) {
                            if (isUnder60) resignedPrevLte60[w - 1]++;
                            else resignedPrevGt60[w - 1]++;
                        }
                        if (mKey === selMonth) {
                            if (isUnder60) resignedCurrLte60[w - 1]++;
                            else resignedCurrGt60[w - 1]++;
                        }
                    }
                }
            });
            ...
            // Resigned Chart Prev
            const elResPrev = document.getElementById('resignedChartPrev');
            if (elResPrev) {
                if (resignedChartPrevInstance) resignedChartPrevInstance.destroy();
                resignedChartPrevInstance = new Chart(elResPrev.getContext('2d'), {
                    type: 'bar',
                    data: {
                        labels: labelsPrev,
                        datasets: [
                            { label: 'Nghỉ trước <= 2 tháng', data: resignedPrevLte60, backgroundColor: '#2563eb', borderRadius: 4 },
                            { label: 'Nghỉ sau > 2 tháng', data: resignedPrevGt60, backgroundColor: '#60a5fa', borderRadius: 4 }
                        ]
                    },
                    options: {
                        responsive: true,
                        maintainAspectRatio: false,
                        plugins: { legend: { position: 'top', labels: { boxWidth: 12, color: 'var(--text-main)' } } },
                        scales: {
            ...
            // Resigned Chart Curr
            const elResCurr = document.getElementById('resignedChartCurr');
            if (elResCurr) {
                if (resignedChartCurrInstance) resignedChartCurrInstance.destroy();
                resignedChartCurrInstance = new Chart(elResCurr.getContext('2d'), {
                    type: 'bar',
                    data: {
                        labels: labelsCurr,
                        datasets: [
                            { label: 'Nghỉ trước <= 2 tháng', data: resignedCurrLte60, backgroundColor: '#2563eb', borderRadius: 4 },
                            { label: 'Nghỉ sau > 2 tháng', data: resignedCurrGt60, backgroundColor: '#60a5fa', borderRadius: 4 }
                        ]
                    },
                    options: {
                        responsive: true,
        ...
        function renderHRDetailTable(activeList, resignedList) {
            const getWeekOfMonthFromStr = (dStr) => {
                if (!dStr) return 0;
                const parts = dStr.split('/');
                if (parts.length === 3) {
            ...
            const tblContainer = document.getElementById('hr-table-container');
            if (!tbody || !tblContainer) return;
            tbody.innerHTML = '';
            tblContainer.style.display = 'block';

            const selCriteria = document.getElementById('hr-filter-criteria')?.value || 'all';
            ...
            } else if (selCriteria === 'active_only') {
                combined = activeList;
            ...
            combined.sort((a,b) => (b.daysWorked||0) - (a.daysWorked||0));

            combined.forEach(e => {
                const tr = document.createElement('tr');
                const isResigned = e.status === 'Đã nghỉ việc';
                tr.innerHTML = `
                    <td>${e.id || ''}</td>
                    ...
                    <td>${e.shift || ''}</td>
                    <td>${e.joinDateStr || ''}</td>
                    <td><span class="badge ${isResigned ? 'badge-danger' : 'badge-success'}">${e.status || 'Đang làm việc'}</span></td>
                    <td>${e.daysWorked || 0} ngày</td>
                `;
                tbody.appendChild(tr);
            });
        }
        ...
        function showActiveModal() {
            if (!currentFilteredActive || currentFilteredActive.length === 0) {
                alert(unescape('Hi%u1EC7n%20kh%F4ng%20c%F3%20nh%E2n%20s%u1EF1%20n%E0o%20%u0111ang%20l%E0m%20vi%u1EC7c%20%28theo%20b%u1ED9%20l%u1ECDc%20hi%u1EC7n%20t%u1EA1i%29.'));
                return;
            }
            let html = '';
            const sorted = [...currentFilteredActive].sort((a,b) => b.daysWorked - a.daysWorked);
            ...
            sorted.forEach(e => {
                html += `
                    <tr>
                        ...
                        <td style="color: var(--success); font-weight:600;">${e.joinDateStr}</td>
                        <td style="color: var(--primary); font-weight: 800; font-size: 1.1rem;">${e.daysWorked} ngày</td>
                    </tr>
                `;
            });
            
            document.getElementById('hr-active-modal-body').innerHTML = html;
            document.getElementById('hr-active-modal').classList.add('active');
        }

        function closeActiveModal() {
            document.getElementById('hr-active-modal').classList.remove('active');
        }
        ...
        // ==========================================
        function calcShiftProgress(startH, endH, now) {
            ...
            if (!isActive) {
                return { isActive: false, percent: 0, leftStr: "Ngoài ca" };
            }

            let startMs = new Date(now).setHours(startH, 0, 0, 0);
            let endMs = new Date(now).setHours(endH, 0, 0, 0);
            ...
            const leftMs = endMs - now.getTime();
            let leftStr = "Sắp xong";
            if (leftMs > 0) {
                const leftH = Math.floor(leftMs / (1000 * 60 * 60));
                const leftM = Math.floor((leftMs % (1000 * 60 * 60)) / (1000 * 60));
                leftStr = `Còn: ${leftH}h${leftM}m`;
            }

            return { isActive: true, percent, leftStr };
        }
        ...
        function updateTracker() {
            ...
            const timeEl = document.getElementById('tracker-current-time');
            if(timeEl) timeEl.textContent = timeStr;

            // Ca 3: 22:00 - 07:00
            const ca3 = calcShiftProgress(22, 7, now);
            const ca3Progress = document.getElementById('tracker-ca3-progress');
            const ca3Percent = document.getElementById('tracker-ca3-percent');
            const ca3TimeLeft = document.getElementById('tracker-ca3-time-left');
            const ca3Name = document.getElementById('tracker-ca3-name');
            if (ca3Progress) {
                ca3Progress.style.width = ca3.percent.toFixed(1) + "%";
                ca3Percent.textContent = ca3.percent.toFixed(1) + "%";
                ca3TimeLeft.textContent = ca3.leftStr;
                if (!ca3.isActive) {
                    ca3Name.innerHTML = `<i class="fa-regular fa-clock" style="color:var(--text-muted)"></i> Ca 3 (22:00 - 07:00)`;
                } else {
                    ca3Name.innerHTML = `<i class="fa-solid fa-bolt" style="color:var(--warning)"></i> Ca 3 (22:00 - 07:00)`;
                }
            }

            // Ca 5: 20:00 - 06:00
            const ca5 = calcShiftProgress(20, 6, now);
            const ca5Progress = document.getElementById('tracker-ca5-progress');
            const ca5Percent = document.getElementById('tracker-ca5-percent');
            const ca5TimeLeft = document.getElementById('tracker-ca5-time-left');
            const ca5Name = document.getElementById('tracker-ca5-name');
            if (ca5Progress) {
                ca5Progress.style.width = ca5.percent.toFixed(1) + "%";
                ca5Percent.textContent = ca5.percent.toFixed(1) + "%";
                ca5TimeLeft.textContent = ca5.leftStr;
                if (!ca5.isActive) {
                    ca5Name.innerHTML = `<i class="fa-regular fa-clock" style="color:var(--text-muted)"></i> Ca 5 (20:00 - 06:00)`;
                } else {
                    ca5Name.innerHTML = `<i class="fa-solid fa-moon" style="color:var(--info)"></i> Ca 5 (20:00 - 06:00)`;
                }
            }
        }
        ...
        function loadScript(src) {
            return new Promise((resolve, reject) => {
                ...
                s.src = src;
                s.onload = resolve;
                s.onerror = () => reject(new Error(`Không thể tải thư viện từ: ${src}`));
                document.head.appendChild(s);
            });
        }

        // ==========================================
        // EXPORT PDF TÍNH NĂNG MỚI (Đa trang & Tự động nạp thư viện)
        // ==========================================
        async function exportDashboardPDF() {
            const btn = document.querySelector('button[onclick="exportDashboardPDF()"]');
            const oldContent = btn ? btn.innerHTML : '';
            if(btn) btn.innerHTML = '<i class="fa-solid fa-spinner fa-spin"></i> Đang chuẩn bị thư viện...';
            ...
            try {
                ...
                if(btn) btn.innerHTML = '<i class="fa-solid fa-spinner fa-spin"></i> Đang chụp báo cáo...';

                let targetEl = null;
                if (document.getElementById('tab-operations') && document.getElementById('tab-operations').classList.contains('active')) {
                    targetEl = document.getElementById('tab-operations');
                } else if (document.getElementById('tab-freelancer') && document.getElementById('tab-freelancer').classList.contains('active')) {
                    targetEl = document.getElementById('tab-freelancer');
                } else if (document.getElementById('tab-hr') && document.getElementById('tab-hr').classList.contains('active')) {
                    targetEl = document.getElementById('tab-hr');
                } else {
                    alert("Tính năng Xuất PDF hỗ trợ cho các tab Nhân Sự, Vận Hành và Freelancer.");
                    if(btn) btn.innerHTML = oldContent;
                    return;
                }

                // Tạm thời bỏ viền input cho ảnh chụp đẹp hơn
                const inputs = targetEl.querySelectorAll('input, select');
                const origBorders = [];
                inputs.forEach(i => {
                    origBorders.push(i.style.border);
                    i.style.border = 'none';
                });
                ...
                inputs.forEach((i, idx) => i.style.border = origBorders[idx]);

                if(btn) btn.innerHTML = '<i class="fa-solid fa-spinner fa-spin"></i> Đang xuất file PDF...';
                ...
                pdf.save(`Bao_Cao_Ngan_AI_${new Date().getTime()}.pdf`);
                
                if(btn) btn.innerHTML = oldContent;
            } catch (e) {
                console.error("Lỗi xuất PDF:", e);
                alert("Lỗi xuất PDF: " + (e.message || "Không thể chụp màn hình. Vui lòng kiểm tra kết nối mạng."));
                if(btn) btn.innerHTML = oldContent;
            }
        }
